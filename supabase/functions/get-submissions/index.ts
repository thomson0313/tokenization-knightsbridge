...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: submissions, error: submissionsError } = await supabaseClient.from('form_submissions').select(`
        *,
    ...
    const transformedSubmissions = submissions.map((submission)=>({
        ...
        uploadedDocuments: submission.uploaded_documents?.map((doc)=>({
            id: doc.id,
